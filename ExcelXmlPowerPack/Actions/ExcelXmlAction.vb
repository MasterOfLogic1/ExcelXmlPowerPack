...
Namespace ExcelXmlMain
    ...
    Public Class ExcelXmlAction
            ...
            Try
                Using document As SpreadsheetDocument = SpreadsheetDocument.Open(_filePath, False)
                    ...
                    If sheet IsNot Nothing Then
                        ...
                        Dim lastUsedRowIndex As Integer = 0

                        For Each row As Row In sheetData.Elements(Of Row)()
                            If row.Elements(Of Cell)().Any(Function(c) c.CellValue IsNot Nothing AndAlso Not String.IsNullOrEmpty(c.CellValue.Text)) Then
                                lastUsedRowIndex = CInt(row.RowIndex.Value)
                            End If
                        Next

                        Return lastUsedRowIndex
                    Else
                        Throw New SystemException("No sheet named " + sheetName.ToString() + " found")
                    End If
                    document.Dispose()
                End Using
            Catch ex As Exception
                    ...
                    If sheet IsNot Nothing Then
                        ...
                        For Each row As Row In sheetData.Elements(Of Row)()
                            For Each cell As Cell In row.Elements(Of Cell)()
                                ...
                                Dim columnIndex As Integer = ExcelXmlHelperActions.GetColumnIndex(cell.CellReference.Value)
        ...
        Public Function GetUsedRange(sheetName As String) As Object() '(topLeft As String, bottomRight As String)
            Dim errorMessage As String = String.Empty
            Try
                Dim firstColumn As Integer = Integer.MaxValue
                Dim lastColumn As Integer = Integer.MinValue
                Dim lastRow As Integer
                Using spreadsheetDocument As SpreadsheetDocument = SpreadsheetDocument.Open(_filePath, False)
                    lastRow = CInt(ExcelXmlHelperActions.GetLastRow(spreadsheetDocument, sheetName))
                    ...
                    If sheetData IsNot Nothing Then
                        For Each row As Row In sheetData.Elements(Of Row)()
                            For Each cell As Cell In row.Elements(Of Cell)()
                                ...
                                Dim column As Integer = ExcelXmlHelperActions.GetColumnIndex(cellReference)
                ...
                If firstColumn <= lastColumn AndAlso lastRow > 0 Then
                    Dim startCell As String = ExcelXmlHelperActions.ColumnNumberToLetter(firstColumn) & "1"
                    Dim endCell As String = ExcelXmlHelperActions.GetColumnName(lastColumn) & lastRow.ToString()
                    Return New Object() {startCell, endCell}
                End If
            Catch ex As Exception
                errorMessage = ex.Message
            End Try
        ...
        'This does not return anything but deletes a target sheet
        ...
        ' This does not return anything but adds color to a specific range in a sheet if no range is selected it paints the entire used range
        Public Sub AddColorToRange(sheetName As String, colorHex As String, Optional cellRange As String = Nothing)
            Dim errorMessage As String = String.Empty
            Try
                Using document As SpreadsheetDocument = SpreadsheetDocument.Open(_filePath, True)
                    ...
                    If sheet IsNot Nothing Then
                        ...
                        Dim startRow As Integer = CInt(ExcelXmlHelperActions.ExtractNumbersFromCellReference(startCell))
                        Dim endColumn As String = ExcelXmlHelperActions.GetColumnName(endCell)
                        Dim endRow As Integer = CInt(ExcelXmlHelperActions.ExtractNumbersFromCellReference(endCell))
        ...
        Public Sub DeleteRange(sheetName As String, Optional cellRange As String = Nothing)
            Dim errorMessage As String = String.Empty
            Try
                Using document As SpreadsheetDocument = SpreadsheetDocument.Open(_filePath, True)
                    Dim startCell, endCell As String
        ...
        Public Function ReadSheetToDataTable(sheetName As String, Optional cellRange As String = Nothing, Optional hasHeader As Boolean = False) As System.Data.DataTable
            ...
            Using document As SpreadsheetDocument = SpreadsheetDocument.Open(_filePath, False)
                ...
                Dim startCol As Integer = ExcelXmlHelperActions.ColumnLetterToNumber(ExcelXmlHelperActions.ExtractLettersFromCellReference(startCell))
                Dim endCol As Integer = ExcelXmlHelperActions.ColumnLetterToNumber(ExcelXmlHelperActions.ExtractLettersFromCellReference(endCell))
                Dim startRow As Integer = Integer.Parse(ExcelXmlHelperActions.ExtractNumbersFromCellReference(startCell))
                Dim endRow As Integer = Integer.Parse(ExcelXmlHelperActions.ExtractNumbersFromCellReference(endCell))
                ...
                Else
                    ...
                    For colNumber As Integer = startCol To endCol
                        ...
                        Dim cell = firstRow.Elements(Of Cell)().FirstOrDefault(Function(c) ExcelXmlHelperActions.ColumnLetterToNumber(ExcelXmlHelperActions.ExtractLettersFromCellReference(c.CellReference)) = cn)
                ...
                For Each row As Row In rows
                    ...
                    For colNumber As Integer = startCol To endCol
                        Dim cn As Integer = colNumber
                        Dim cell = row.Elements(Of Cell)().FirstOrDefault(Function(c) ExcelXmlHelperActions.ColumnLetterToNumber(ExcelXmlHelperActions.ExtractLettersFromCellReference(c.CellReference)) = cn)
                        If cell IsNot Nothing Then
        ...
        'static or shared Function to write a sheet to a table : does same thing as write datatable but this time specifies where to start writing from

        Public Shared Sub WriteDataTableToSheet(filePath As String, sheetName As String, dataTable As System.Data.DataTable, Optional startCell As String = "A1", Optional AddHeader As Boolean = True)
            ...
            If Not String.IsNullOrEmpty(startCell) Then
                Dim cellReference As String = startCell.ToUpper()
                startColumn = ExcelXmlHelperActions.GetColumnIndex(cellReference)
                startRow = CInt(ExcelXmlHelperActions.ExtractNumbersFromCellReference(cellReference))
            End If
            ...
            If AddHeader Then
                Dim headerRow As New Row With {.RowIndex = CUInt(startRow)}
                Dim currentColumn As Integer = startColumn
                For Each column As DataColumn In dataTable.Columns
                    Dim cell As New Cell With {
                .CellReference = ExcelXmlHelperActions.GetCellReference(currentColumn, startRow),
                ...
                currentRow += 1 ' Increment row index to start writing data below headers
            End If

            ' Write the rows
            For Each dataRow As DataRow In dataTable.Rows
                Dim newRow As New Row With {.RowIndex = CUInt(currentRow)}
                Dim currentColumn As Integer = startColumn
                For Each column As DataColumn In dataTable.Columns
                    Dim cellValue As String = If(dataRow(column) IsNot DBNull.Value, dataRow(column).ToString(), String.Empty)
                    Dim cell As New Cell With {
                .CellReference = ExcelXmlHelperActions.GetCellReference(currentColumn, currentRow),
        ...
        'static or shared Function to append a table to a sheet
        Public Shared Sub AppendDataTableToSheet(filePath As String, sheetName As String, dataTable As System.Data.DataTable)
            ' Open or create the spreadsheet document
            Dim spreadsheetDocument As SpreadsheetDocument
            If IO.File.Exists(filePath) Then
                spreadsheetDocument = SpreadsheetDocument.Open(filePath, True)
            ...
            ' Get the SheetData from the worksheet
            Dim sheetData As SheetData = worksheetPart.Worksheet.GetFirstChild(Of SheetData)()
            Dim lastRow As String = ExcelXmlHelperActions.GetLastRow(spreadsheetDocument, sheetName) + 1
            ...
            Else
                AddHeader = False
            End If
            Dim startCell As String = "A" + lastRow
            ' Determine the starting row and column
            Dim startRow As Integer = 1
            Dim startColumn As Integer = 1
            If Not String.IsNullOrEmpty(startCell) Then
