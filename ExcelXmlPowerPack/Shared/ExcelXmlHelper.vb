...
Namespace ExcelXmlHelper
    Public Class ExcelXmlHelperActions
        ...
        'Helper function to get column name from cell reference i.e returns A when A20 is eneterd
        ...
        'Overide Helper function to get column name from column index i.e returns A when 1 is eneterd

        Public Shared Function GetColumnName(columnIndex As Integer) As String
            Dim columnName As String = String.Empty
            While columnIndex > 0
                columnIndex -= 1
                columnName = ChrW(columnIndex Mod 26 + AscW("A"c)) & columnName
                columnIndex \= 26
            End While
            Return columnName
        End Function
        ...
        Public Shared Function GetColumnIndex(cellReference As String) As Integer
        ...
        'helper function to get cell reference from column and row number
        Public Shared Function GetCellReference(column As Integer, row As Integer) As String
            Dim columnName As String = GetColumnName(column)
        ...
        'helper function aids recognition of date format in excel
        ...
        Public Shared Function ExtractLettersFromCellReference(cellReference As String) As String
        ...
        Public Shared Function ExtractNumbersFromCellReference(cellReference As String) As String
        ...
        Public Shared Sub CheckRangeValidity(cellRange As String)
            Try
                ...
                ValidateCellReference(rangeParts(0))
                ValidateCellReference(rangeParts(1))
        ...
        Private Shared Sub ValidateCellReference(cellReference As String)
            ' Ensure the cell reference has at least one letter and one number
            If String.IsNullOrEmpty(cellReference) OrElse Not cellReference.Any(AddressOf Char.IsLetter) OrElse Not cellReference.Any(AddressOf Char.IsDigit) Then
                Throw New ArgumentException($"Invalid cell reference: {cellReference}")
            End If

            ' Extract column letters and row numbers
            Dim letters As String = ExcelXmlHelperActions.ExtractLettersFromCellReference(cellReference)
            Dim numbers As String = ExcelXmlHelperActions.ExtractNumbersFromCellReference(cellReference)
            ...
            If colNumber < 1 OrElse colNumber > 16384 Then
                Throw New ArgumentException($"Invalid column in cell reference: {cellReference}")
            End If
            ...
            If rowNumber < 1 OrElse rowNumber > 1048576 Then
                Throw New ArgumentException($"Invalid row in cell reference: {cellReference}")
            End If
        End Sub
